...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1).detach()]
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, finetune_fc=False):
        if finetune_fc:
            with torch.no_grad():
                patch_emds = self.forward_return_n_last_blocks(x, self.n_last_blocks, depth = self.depths)
        else:
            patch_emds = self.forward_return_n_last_blocks(x, self.n_last_blocks, depth = self.depths)
        output = self.head(patch_emds)
        output = torch.nn.functional.normalize(output, p=2, dim=1)  # L2 normalize
